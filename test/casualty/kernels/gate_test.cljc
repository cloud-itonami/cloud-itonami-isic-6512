(ns casualty.kernels.gate-test
  ...
  3. governor boundary — the confidence floor boundary and the
     ...
     through the real `casualty.governor/check` façade."
...
(deftest bind-and-settle-auto-enabled-nowhere
  (testing "op 4 (:policy/bind) and op 6 (:claim/settle) are auto-enabled at
...
;; ---------------------------------------------------------------
;; Governor boundary through the real façade. op :policy/intake
;; touches neither the store nor the docs/spec/claim checks, so the
;; verdict is decided purely by confidence/actuation — nil store is
;; safe.

(defn- verdict [proposal]
  (governor/check {:op :policy/intake :subject "pol-x"} {} proposal nil))
...
(deftest actuation-still-escalates-and-hard-still-wins
  (is (true? (:escalate? (verdict {:confidence 0.99 :stake :actuation/bind}))))
  (is (true? (:escalate? (verdict {:confidence 0.99 :stake :actuation/settle-claim}))))
  (testing "a hard violation dominates actuation escalation"
    (let [v (governor/check {:op :jurisdiction/assess :subject "pol-x"} {}
                            {:confidence 0.99 :stake :actuation/bind :cites []} nil)]
      (is (true? (:hard? v)))
      (is (false? (:escalate? v)))
