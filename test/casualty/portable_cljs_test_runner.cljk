(ns casualty.portable-cljs-test-runner
  ...
  `casualty.store-contract-test`, which exercises the langchain.db
  ...
  -m if combined, hence -Sdeps for the extra path):
    ...
    clojure -Sdeps '{:paths [\"src\" \"test\"]}' \\
      ...
      --output-to target/tests.cjs -c casualty.portable-cljs-test-runner
  ...
  (:require [clojure.test :as t :refer [run-tests]]
            [casualty.corporate-intel-test]
            [casualty.facts-test]
            [casualty.governor-contract-test]
            [casualty.kernels.gate-test]
            [casualty.phase-test]
            [casualty.registry-test]
            [casualty.store-contract-test]))
