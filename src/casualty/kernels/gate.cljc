(ns casualty.kernels.gate
  "Safety kernel for the non-life-insurance governor + phase gate — the
  decision CORE of `casualty.governor/check` and `casualty.phase/gate`,
  extracted into the safe-kotoba subset (cloud-itonami kernels
  discipline, ADR-0016 / superproject ADR-2607101200), the same pattern
  ...
  (`casualty.governor`, `casualty.phase`) reduce their inputs to
  ...
  Wire codes:
    ...
    op          0 read (reserved — this actor has NO read ops today, so
                  ...
                  sibling kernels)
                1 :policy/intake            2 :jurisdiction/assess
                3 :kyc/screen               4 :policy/bind
                5 :claim/file               6 :claim/settle
  ...
  LESS autonomy (hold/escalate), never more. `:policy/bind` (op 4) and
  `:claim/settle` (op 6) — the two real-world legal/financial acts —
...
(defn or7 [a b c d e f g] (or2 (or3 a b c) (or2 (or3 d e f) g)))
...
(defn hard-violation
  ...
  spec-basis missing / sanctions hit / required documents incomplete /
  claim filed against an unbound policy / settlement of a nonexistent
  claim / settlement exceeding the policy's own coverage limit /
  double settlement of an already-settled claim."
  [spec-missing sanctions-hit docs-incomplete policy-not-bound
   claim-missing claim-exceeds claim-settled]
  (or7 (norm-flag spec-missing)
       (norm-flag sanctions-hit)
       (norm-flag docs-incomplete)
       (norm-flag policy-not-bound)
       (norm-flag claim-missing)
       (norm-flag claim-exceeds)
       (norm-flag claim-settled)))

(defn verdict-code
  "Governor verdict: 2 hard-hold wins over 1 escalate wins over 0 ok."
  [spec-missing sanctions-hit docs-incomplete policy-not-bound
   claim-missing claim-exceeds claim-settled confidence-x100 actuation]
  (if (= 1 (hard-violation spec-missing sanctions-hit docs-incomplete
                           policy-not-bound claim-missing claim-exceeds
                           claim-settled))
    2
...
(defn op-auto-enabled
  ...
  column). Exactly two cells are ever 1: phase 3 x :policy/intake and
  phase 3 x :claim/file (neither moves capital yet). op 4
  (:policy/bind) and op 6 (:claim/settle) are 0 at every phase —
  permanent structural fact, not a rollout milestone."
  [phase op]
  (if (= phase 3) (if (= op 1) 1 (if (= op 5) 1 0)) 0))

(defn phase-disposition
  "Resolve the final disposition code from phase, op code and the
  governor's disposition code. Mirrors `casualty.phase/gate`:
...
(defn check-verdict [spec sanc docs unbound missing exceeds settled conf act expected]
  (if (= (verdict-code spec sanc docs unbound missing exceeds settled conf act)
         expected)
    1
    0))
...
(defn battery-pass-count []
  (+
   ...
   (check-verdict 1 1 0 0 1 0 0 100 0 2)
   ...
   (check-verdict 0 0 0 0 0 0 3 100 0 2)
