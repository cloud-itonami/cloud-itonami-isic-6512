(ns casualty.operation
  "OperationActor -- one non-life-insurance operation = one supervised
  actor run, expressed as a langgraph-clj StateGraph. The advisor
  (Underwriter-LLM) is sealed into a single node (:advise); its proposal
  is ALWAYS routed through the Non-Life Insurance Governor (:govern) and
  ...
  One graph run = one non-life-insurance operation (intake -> advise ->
  ...
  decision to a human operator (the licensed underwriter/adjuster). The
  approver resumes with `{:approval {:status :approved}}` (or
  :rejected). `:policy/bind`/`:claim/settle` ALWAYS reach this node when
  the governor is clean -- see `casualty.phase`."
  (:require [langgraph.graph :as g]
            [langgraph.checkpoint :as cp]
            [casualty.underwriterllm :as underwriterllm]
            [casualty.governor :as governor]
...
(defn build
  ...
  `casualty.store/Store`).
  opts:
    :advisor      -- a `casualty.underwriterllm/Advisor` (default: mock-advisor)
    :checkpointer -- langgraph checkpointer (default: in-mem)"
  [store & [{:keys [advisor checkpointer]
             :or   {advisor      (underwriterllm/mock-advisor)
  ...
  (-> (g/state-graph
      ...
      ;; Underwriter-LLM inference (the contained intelligence node) -- proposal only.
      (g/add-node :advise
        (fn [{:keys [request]}]
          (let [p (underwriterllm/-advise advisor store request)]
            {:proposal p :audit [(underwriterllm/trace request p)]})))

      ;; Non-Life Insurance Governor -- independent censor (separate system than the LLM).
      (g/add-node :govern
